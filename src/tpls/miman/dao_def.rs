...
{{#if (nep pk_name)}}func (dao *{{dao_name}}) GetById(session *gorm.DB, id {{pk_type}}) (*do.{{entity_name}}, error) {
...
{{#if (nep pk_name)}}func (dao *{{dao_name}}) GetByIdList(session *gorm.DB, idList []{{pk_type}}) (do.{{entity_list_name}}, error) {
